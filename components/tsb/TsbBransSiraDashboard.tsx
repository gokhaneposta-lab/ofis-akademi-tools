...
export default function TsbBransSiraDashboard() {
  ...
  const sirketler = useMemo(() => {
    ...
    return listSirketlerSiraOzeti(rows, secilenDonem, kanal);
  ...
  const tablo = useMemo(() => {
    ...
    return buildBransSiraTablosu(rows, secilenDonem, kanal, effectiveSirketKodu);
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-[11px] leading-relaxed text-gray-600">
        <strong>Sıra:</strong> İlgili branşta seçilen kanaldaki prim üretimine göre sektör içi yarışma sırası (1 en yüksek
        prim). <strong>Δ sıra:</strong> Önceki yıla göre sıra farkı; sayının <span className="text-emerald-700">azalması</span>{" "}
        iyileşmedir (↑ gösterilir). Üretim yoksa (prim 0) sıra gösterilmez.
      </p>
